...
const ApplicationForm = ({ onComplete }: { onComplete: () => void }) => {
  ...
  return (
    <motion.div 
      ...
      className="w-full bg-app-bg rounded-3xl border border-black/5 shadow-lg mt-4 p-8"
    >
      <div className="mb-8">
        ...
        {step === 3 && (
          ...
          >
            <div className="space-y-4">
              ...
              <div className="p-4 bg-ai-tint rounded-2xl border border-brand/15 flex items-start gap-3">
...
const ContractSummary = ({ property, onComplete }: { property: Property, onComplete?: () => void }) => {
  ...
  if (isPaid) {
    return (
      <motion.div
        ...
        className="w-full bg-gradient-to-br from-ai-tint to-surface-2 rounded-3xl border border-brand/20 shadow-2xl mt-4 overflow-hidden"
      >
  ...
  return (
    <motion.div
      initial={{ opacity: 0, y: 20 }}
      animate={{ opacity: 1, y: 0 }}
      className="w-full bg-app-bg rounded-3xl border border-black/5 shadow-2xl mt-4 overflow-hidden"
    >
      <div className="p-8 pb-4">
        <div className="flex items-center justify-between mb-4">
          <div className="text-xs font-medium text-black uppercase tracking-wider">Official Agreement</div>
          <div className="flex items-center gap-1.5 px-2 py-1 bg-ai-tint text-brand rounded-full">
    ...
    >
      {step === 'analyzing' ? (
        ...
        <div className="p-8 space-y-8">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 bg-ai-tint rounded-2xl flex items-center justify-center text-brand text-2xl">
      ...
      <div className="p-6 space-y-4">
        {tasks.map((task) => (
          <div 
            ...
            className={`flex items-center justify-between p-4 rounded-2xl border transition-all cursor-pointer ${task.completed ? 'bg-ai-tint border-brand/15' : 'bg-neutral-50 border-neutral-100 hover:border-black/20'}`}
          >
            <div className="flex items-center gap-3">
              <div className={`${task.completed ? 'text-brand' : 'text-neutral-400'}`}>
                {task.icon}
              </div>
              <span className={`text-sm font-bold ${task.completed ? 'text-brand-heading line-through opacity-60' : 'text-black'}`}>
                {task.label}
              </span>
            </div>
  ...
  if (confirmed) {
    ...
    return (
      <motion.div
        initial={{ opacity: 0, scale: 0.95 }}
        animate={{ opacity: 1, scale: 1 }}
        className="w-full bg-ai-tint rounded-3xl border border-brand/15 shadow-lg mt-4 p-8 text-center"
      >
...
const ChatInterface: React.FC<ChatInterfaceProps> = ({
  ...
  const prevPrefCountRef = useRef((userPreferences ?? []).length);
  const scrollRef = useRef<HTMLDivElement>(null);
  const inputRef = useRef<HTMLTextAreaElement>(null);
  const suggestionsRef = useRef<HTMLDivElement>(null);
  // Default to true so the right scroll affordance shows from the first
  // render when the suggestion row overflows. Recomputed by checkSuggestionsScroll.
  const [canScrollLeft, setCanScrollLeft] = useState(false);
  const [canScrollRight, setCanScrollRight] = useState(true);

  // Detect new preferences and show celebration toast
  useEffect(() => {
    const currentCount = (userPreferences ?? []).length;
    if (currentCount > prevPrefCountRef.current && prevPrefCountRef.current > 0) {
      const newPref = (userPreferences ?? [])[currentCount - 1];
      const filledCats = new Set((userPreferences ?? []).map(p => p.category)).size;
      const messages = [
        `+1 ${newPref?.label ?? 'preference'}!`,
        `${filledCats}/8 categories filled!`,
        'Getting closer to your dream home!',
        'Nice! Keep going!',
      ];
      const msg = filledCats === 8 ? 'Profile complete!' : messages[Math.min(filledCats - 1, messages.length - 1)];
      setPrefToast(msg);
      const timer = setTimeout(() => setPrefToast(null), 2500);
      return () => clearTimeout(timer);
    }
    prevPrefCountRef.current = currentCount;
  }, [userPreferences]);
  ...
  const showContextChip = contextProperty && !isCollapsed && contextProperty.id !== dismissedContextPropertyId;
   ...
   return (
    <div className="flex-1 min-h-0 flex flex-col w-full relative h-full overflow-hidden bg-app-bg">
       {!isCollapsed && (
               ...
               <div key={msg.id} className="animate-fade-in-up">
                 ...
                 {msg.role === 'user' ? (
                   ...
                   <div className="flex flex-col gap-3 w-full">
                     ...
                     <div className={`text-neutral-800 text-sm leading-relaxed pl-1 whitespace-pre-wrap font-medium ${msg.isSigningMessage ? 'bg-ai-tint p-6 rounded-2xl border-2 border-emerald-500/20 shadow-xl shadow-emerald-500/5' : ''}`}>
                        ...
                        {msg.isSigningMessage && !msg.interactiveType && (
                          <div className="mt-8 p-6 bg-white rounded-xl border border-brand/15 flex items-center justify-between shadow-sm">
                             ...
                             <div className="text-xs font-medium text-brand uppercase tracking-wider bg-ai-tint px-3 py-1.5 rounded-full">Active</div>
                          </div>
                        )}
                      </div>
       ...
       <div className={`w-full px-4 flex flex-col items-center justify-end z-50 shrink-0 absolute bottom-0 left-0 right-0 bg-gradient-to-t from-app-bg via-app-bg/90 to-transparent ${!isCollapsed ? 'pb-4 pt-8' : 'pb-4 pt-0'}`}>
            ...
            <div className="relative w-full max-w-3xl mb-2">
              ...
              {canScrollLeft && (
                <button
                  ...
                  className="absolute left-0 top-1/2 -translate-y-1/2 z-10 w-8 h-8 bg-white shadow-md border border-black/5 rounded-full flex items-center justify-center text-black hover:bg-black hover:text-white transition-all"
                >
                  <ChevronLeft size={16} />
                </button>
              )}
           ...
           <div className={`bg-white rounded-2xl shadow-[0_10px_40px_rgba(0,0,0,0.08)] border border-black/5 transition-all duration-500 focus-within:ring-1 focus-within:ring-black focus-within:shadow-[0_20px_50px_rgba(0,0,0,0.12)] flex flex-col gap-1 ${isCollapsed ? 'p-2 min-h-[3rem]' : 'p-3 min-h-[5rem]'}`}>
             ...
             <div className={`flex items-center justify-between mt-1 pt-1 ${isCollapsed ? 'hidden' : ''}`}>
               <div className="flex items-center gap-1">
                 {/* Renter profile / preference indicator */}
                 <div className="relative">
                   ...
                   <AnimatePresence>
                     {showPrefsPopover && (
                       <RenterProfilePopover
                         userPreferences={userPreferences}
                         isLoggedIn={isLoggedIn}
                         onPreferenceLoginPrompt={onPreferenceLoginPrompt}
                         onClose={() => setShowPrefsPopover(false)}
                         onSendMessage={(text) => {
                           onSendMessage(text);
                         }}
                 ...
                 {onResetChat && messages.length > 0 && (
                   <div className="relative">
                     <button
                       ...
                       title="Reset chat"
                       className={`p-1.5 rounded-full transition-all ${showResetConfirm ? 'bg-neutral-100 text-black' : 'text-neutral-300 hover:text-neutral-500 hover:bg-neutral-100'}`}
                     >
                       <RotateCcw size={14} />
                     </button>
                     <AnimatePresence>
