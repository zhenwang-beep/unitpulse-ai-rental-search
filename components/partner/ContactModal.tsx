    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-0 sm:p-6">
      ...
      <div className="relative w-full sm:max-w-lg h-full sm:h-auto bg-white sm:rounded-2xl shadow-2xl overflow-hidden flex flex-col sm:max-h-[90vh] animate-[pulse_0.2s_ease-out]">
        ...
        <div className="px-8 py-6 border-b border-black/5 flex justify-between items-center bg-surface-2">
          ...
          <button
            onClick={onClose}
            className="w-8 h-8 rounded-full bg-white border border-black/5 flex items-center justify-center text-neutral-400 hover:text-black hover:border-black transition-colors"
          >
            <X size={16} />
          </button>
        </div>
        ...
        <div className="p-8 overflow-y-auto custom-scrollbar flex-1">
          {status === 'success' ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <div className="w-16 h-16 bg-ai-tint rounded-full flex items-center justify-center text-brand-hover mb-6">
                <CheckCircle2 size={32} />
              </div>
              <h4 className="text-2xl font-heading text-black mb-2">Request Received</h4>
              <p className="text-neutral-500 max-w-xs mb-8 mx-auto">
                Thanks {formData.name.split(' ')[0]}! We'll be in touch shortly to schedule your demo.
              </p>
              <button
                onClick={onClose}
                className="px-8 py-3 bg-black text-white text-sm font-bold uppercase tracking-widest rounded-lg hover:bg-neutral-900 transition-colors"
              >
                Close
              </button>
            </div>
            ...
            <form id="contact-form" onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-wider text-neutral-500">Company</label>
                  <input
                    required
                    type="text"
                ...
                <div className="space-y-2">
                  <label className="text-xs font-bold uppercase tracking-wider text-neutral-500">Role</label>
                  <input
                    type="text"
                    required
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold uppercase tracking-wider text-neutral-500">Something specific?</label>
                <textarea
                  rows={3}
                  required
        ...
        {status !== 'success' && (
          <div className="p-6 border-t border-black/5 bg-white">
            <p className="text-[11px] leading-normal text-neutral-500 text-center mb-3">
