...
const ChatPage: React.FC<ChatPageProps> = ({ isLoggedIn, setShowLoginView, setPendingFavoriteProperty, handleLogout, showToast }) => {
  ...
  return (
    <div className="flex flex-col h-[100dvh] w-full bg-surface-app font-sans text-black overflow-hidden">
       ...
       <header className={`fixed top-0 left-0 right-0 px-4 md:px-8 py-4 flex justify-between items-center z-[60] transition-all duration-300 ${isHeaderVisible ? 'translate-y-0' : '-translate-y-full'} ${isPropertyPanelOpen ? 'bg-white border-b border-black/10' : isAtTop ? 'bg-surface-app' : 'bg-white shadow-sm'}`}>
